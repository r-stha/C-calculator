...
class ScientificCalculation 
{
    ...
    public static double SinFunctionLogic(double x)
    {
        ...
        {
            double term = Math.Pow(-1, n) * Math.Pow(x, 2 * n + 1) / ArthmeticCalculation.ComputeFactorialRecursive(2 * n + 1);
            sum += term;
        }
        return sum;
    }
    ...
    public static double CosFunctionLogic(double x)
    {
        ...
        for (int n = 0; n < terms; n++)
        {
            double term = Math.Pow(-1, n) * Math.Pow(x, 2 * n) / ArthmeticCalculation.ComputeFactorialRecursive(2 * n);
            sum += term;
        }
        return sum;
    }
    ...
    public static void CombinationAndPermutation()
    {
        ...
        double combinationResult = ArthmeticCalculation.ComputeFactorialRecursive(totalObjects) / (ArthmeticCalculation.ComputeFactorialRecursive(choosenNumber) * ArthmeticCalculation.ComputeFactorialRecursive(totalObjects - choosenNumber));
        double permutationResult = combinationResult * ArthmeticCalculation.ComputeFactorialRecursive(choosenNumber);
